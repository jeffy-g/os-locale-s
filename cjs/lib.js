...
function execCommand(command, async) {
  if (async) {
    ...
    return /** @type {R} */ (
      new Promise((resolve) => {
        (0, child_process_1.exec)(command, execOpt, (err, stdout) =>
          resolve(err || stdout),
        );
      })
    );
  ...
  try {
    return /** @type {R} */ (
      (0, child_process_1.execSync)(command, sharedExecOpt)
    );
  } catch (e) {
    return /** @type {R} */ (e);
  }
...
const unixProcessor = (result) =>
  (0, exports.purgeExtraToken)(parseLocale(result));
