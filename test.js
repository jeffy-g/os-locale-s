 ...
 * @returns {boolean}
 */
const ifDefined = (varName, fallback) =>
  typeof global[varName] !== "undefined" ? global[varName] : fallback;
// @ts-ignore
...
const printInfo = (prefix, locale, opt) => {
  console.log(
    `${prefix}[platform: ${process.platform}, options: ${opt ? JSON.stringify(opt) : "use default(undefined)"}]: ${locale}`,
  );
};
/**
...
const emitCallback = (localeResult, detectorOpt, async) => async () => {
  ...
  root[
    `options: ${opt ? JSON.stringify(opt).replace(/"/g, "") : "use default(undefined)"}`
  ] = locale;
  ...
  describe(` ====================== running test: [os-locale-s], module - "${path}" ======================`, function () {
    ...
    afterAll(() => {
      ...
      const outputJsonPath =
        logRoot +
        path.replace(/[.]+/g, "dots").replace(/[/\\]/g, "_") +
        "_test-result.json";
      fs.writeFileSync(outputJsonPath, JSON.stringify(localeResult, null, 2));
    });
    describe.each([
      [/** @type {TOSTokens} */ ("linux"), LINUX],
      [/** @type {TOSTokens} */ ("win32"), WIN],
      [/** @type {TOSTokens} */ ("darwin"), DARWIN],
      ...
      if (enable) {
        ...
        describe("locale detection with default options (no cache)", function () {
          it(
            "async detection",
            emitCallback(localeResult, [true, false], true),
          );
        ...
        describe("locale detection with no spawn, no cache", function () {
          it(
            "async detection",
            emitCallback(localeResult, [false, false], true),
          );
    ...
    DEFAULT &&
      describe("[[[ OS: default (**Test depending on the actual platform) ]]]", () => {
        beforeAll(
          setPlatformOf(cache.platform, () => (process.env = cache.env)),
        );
        describe("locale detection with default options", function () {
          it("async detection", emitCallback(localeResult, void 0, true));
          it("sync detection", emitCallback(localeResult));
